...
def main(argv):
    try:
        ...
        sys.exit(2)
    _dict = {"operation":"", "params":""}
    for opt, arg in opts:
        if opt == '-h':
            print('test.py [-m|--migrate] [-i|--invoke invoke.json -f function] [-c swap.cs] ')
            sys.exit()
        elif opt in ("-m", "--migrate"):
            _dict.operation = "migrate"
            deploy_cmd(_dict, str(arg))
        elif opt in ("-i", "--invoke"):
            _dict.operation = "invoke"
            invoke_cmd(_dict, str(arg))
        elif opt in ("-c", "--compile"):
            compile_cmd(_dict, str(arg))
            return
        elif opt in ("-f", "--function"):
            params = str(arg).split(",")
            params1 = params.copy()
            for func in _dict.params.value():
...
def invoke(sdk, m, des_fun_name=None):
    """
    :param sdk:
    :param m:
    :param des_fun_name: the designated function name to be invoked
    :return:
    """
    func_maps = {}
    for function in dict(m["function"]).values():
        if des_fun_name is not None:
            if function.function_name not in des_fun_name:
                continue
        func_map = {}
        param_list = []
